(() => {
    ...
    function spollers() {
        ...
        if (spollersArray.length > 0) {
            ...
            function setSpollerAction(e) {
                ...
                if (link && link.querySelector('input[type="checkbox"]')) {
                    e.preventDefault();
                    const targetCheckbox = link.querySelector('input[type="checkbox"]');
            ...
            function handleCheckboxClick(checkbox) {
                if (checkbox.classList.contains("spollers__parent-checkbox")) {
                    const parentDetails = checkbox.closest("details");
                    const isChecked = checkbox.checked;
                    if (parentDetails && !parentDetails.open) {
                        const summary = parentDetails.querySelector("summary");
                        const spollersBlock = findParentSpollers(parentDetails);
                        if (spollersBlock && spollersBlock.classList.contains("_spoller-init")) {
                            const oneSpoller = spollersBlock.hasAttribute("data-one-spoller");
                            const spollerSpeed = spollersBlock.dataset.spollersSpeed ? parseInt(spollersBlock.dataset.spollersSpeed) : 500;
                            if (oneSpoller) hideSpollersBody(spollersBlock, parentDetails);
                            parentDetails.open = true;
                            summary.classList.add("_spoller-active");
                            _slideToggle(summary.nextElementSibling, spollerSpeed);
                        }
                    }
                    const childCheckboxes = parentDetails.querySelectorAll(".spollers__child-checkbox");
                    childCheckboxes.forEach(child => {
                        child.checked = isChecked;
                    });
                    return;
                }
                if (checkbox.classList.contains("spollers__child-checkbox")) {
                    const parentId = checkbox.dataset.parent;
                    if (parentId) {
                        const parentCheckbox = document.getElementById(parentId);
                        if (parentCheckbox) updateParentCheckboxState(parentCheckbox);
                    }
                    return;
                }
            }
            function updateParentCheckboxState(parentCheckbox) {
                const parentDetails = parentCheckbox.closest("details");
                ...
                on: {
                    init: function() {
                        ...
                        videos.forEach(video => {
                            video.muted = false;
                            const muteButton = document.querySelector(".preview-slider-video-mute");
                            let icon = null;
                            if (muteButton) {
                                icon = muteButton.querySelector("iconify-icon");
                                if (icon) icon.setAttribute("icon", "octicon:unmute-16");
                            }
                            const playVideo = () => {
                                video.play().catch(() => {
                                    console.log("Автозапуск со звуком заблокирован браузером");
                                    video.muted = true;
                                    video.play().catch(() => {});
                                    if (icon) icon.setAttribute("icon", "mdi:mute");
                                });
                            };
                            if (video.readyState >= 2) playVideo(); else {
                            ...
                            video.addEventListener("pause", () => {
                                if (!video.ended && swiper.autoplay) swiper.autoplay.start();
                            });
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (mobileInput) {
            ...
            mobileInput.addEventListener("keydown", function(e) {
                ...
                if (e.key === "Enter") {
                    e.preventDefault();
                    console.log("Выполнить поиск:", this.value);
                }
            });
        }
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        window.addEventListener("scroll", () => {
            if (window.scrollY > 500) upButton.classList.add("show"); else upButton.classList.remove("show");
        });
