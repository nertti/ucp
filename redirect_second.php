...
/*
RewriteRule ^(ok\d+)$ /redirect.php?id=$1 [L,R=301] # редирект на обучающие курсы ФБЖ
RewriteRule ^r/(.*)$ https://online.ucp.by/redirect.php?path=$1 [L,QSA] # редирект на тестирование
RewriteRule ^rd/(.*)$ https://do.ucp.by/redirect.php?path=$1 [L,QSA] # редирект на тестирование
RewriteRule ^rt/(.*)$ https://test.ucp.by/redirect.php?path=$1 [L,QSA] # редирект на тестирование
 */
...
try {
    ...
    #header('Location: https://somesite.com/redirect.php?' . $query);

    if (isset($path) and !empty($path) and $path == 'vks') {
        header('Location: https://webme.by/rooms/ieb-0pf-sam-em6/join'); // Комната 1 (УГЗ МЧС)
        exit;        
    }

    
    if (isset($path) and !empty($path) and $path == 'meet') {
        header('Location: https://webme.by/rooms/c4w-nam-wcv-ino/join'); // Комната 2 (УГЗ)
        exit;        
    }


    if (isset($path) and !empty($path) and $path == 'edi') {
        header('Location: https://webme.by/rooms/5gu-dzm-0cr-1om/join'); // Комната 3 (ЕДИ)
        exit;        
    }
