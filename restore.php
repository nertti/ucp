...
if (LANG == 'ru')
{
	$MESS = [
		...
		"TITLE_PROCESS0" => "Расшифровка архива",
...
elseif (LANG == 'de')
{
	$MESS = [
		...
		"TITLE_PROCESS0" => "Entschlüsseln des Archivs",
...
else
{
	$MESS = [
		...
		"TITLE_PROCESS0" => "Decrypting the archive",
...
if ($Step == 2 && !$bSelectDumpStep)
{
	...
	{
		$file = $_SERVER['DOCUMENT_ROOT'] . '/file_list.xml';
		$str = file_get_contents($file);
		$lastNum = 0;
		if (preg_match_all('#<file name="(' . $_REQUEST['bitrixcloud_backup'] . '.*?)"#', $str, $files))
		{
			foreach ($files[1] as $fileName)
			{
				if (preg_match('/\.(\d+)$/', $fileName, $match))
				{
					if (intval($match[1]) > $lastNum)
					{
						$lastNum = intval($match[1]);
					}
				}
			}
		}
		...
		if (empty($_REQUEST['try_next']) && $strErrMsg)
		{
			$text = $strErrMsg .
				...
				'<input type="hidden" name="bitrixcloud_backup_last_num" value="' . intval($lastNum) . '">' .
	...
	if ($source == 'download' || $source == 'bitrixcloud')
	{
		...
		if ($strUrl)
		{
			...
			if ($arc_name == CTar::getFirstName($arc_name))
			{
				...
				if (isset($_REQUEST['bitrixcloud_backup_last_num']) || isset($lastNum))
				{
					$n = intval($_REQUEST['bitrixcloud_backup_last_num'] ?? $lastNum);
				}
				else
				{
					$tar = new CTar();
			...
			if (!empty($_REQUEST['continue']))
			{
				...
				{
					if ($res == 1 && !preg_match('/\.enc(\.\d+)?$/', $arc_name))
					{
		...
		if ($res)
		{
			...
			if ($res == 2)
			{
				...
				$text .= '<input type="hidden" name="bitrixcloud_backup_last_num" value="' . intval($_REQUEST['bitrixcloud_backup_last_num'] ?? $lastNum) . '">';
			}
			else
			{
				$tar = new CTar();
...
elseif ($Step == 2)
{
	...
	{
		if (substr($arc_name, -4) === '.enc' && $_REQUEST['EncryptKey'] && !isset($_REQUEST['EncryptAlgorithm']))
		{
			$bottom = '<a href="/restore.php?Step=1&lang=' . LANG . '">' . getMsg('BUT_TEXT_BACK') . '</a>';

			// Check for bitrix header block (old archive)
			$arc = fopen($_SERVER['DOCUMENT_ROOT'] . '/' . $arc_name, 'rb');
			$header = fread($arc, 512);
			if (strpos($header, CTar::BX_SIGNATURE) === false)
			{
				$EncryptAlgorithm = CTar::detectEncryptAlgorithm($_SERVER['DOCUMENT_ROOT'] . '/' . $arc_name, $_REQUEST['EncryptKey']);

				$encodedFilesCount = count(glob($_SERVER['DOCUMENT_ROOT'] . '/' . $arc_name . '*'));
				SetCurrentProgress(0, $encodedFilesCount);
			}
			else
			{
				$EncryptAlgorithm = 'skip_decrypt';
			}

			if ($EncryptAlgorithm)
			{
				$hidden = '<input type="hidden" name="EncryptKey" value="' . htmlspecialcharsbx($_REQUEST['EncryptKey']) . '">' .
					'<input type="hidden" name="EncryptAlgorithm" value="' . htmlspecialcharsbx($EncryptAlgorithm) . '">' .
					'<input type="hidden" name="encodedFilesCount" value="' . intval($encodedFilesCount) . '">' .
					'<input type="hidden" name="arc_name" value="' . htmlspecialcharsbx($arc_name) . '">';
				showMsg(getMsg('TITLE_PROCESS0'), $status . $hidden, $bottom);
				?><script>reloadPage(2, 1);</script><?
			}
			else
			{
		...
		// $_REQUEST['EncryptAlgorithm'] detected
		if (substr($arc_name, -4) === '.enc' && $_REQUEST['EncryptKey'] && $_REQUEST['EncryptAlgorithm'] !== 'skip_decrypt')
		{
			$bottom = '<a href="/restore.php?Step=1&lang=' . LANG . '">' . getMsg('BUT_TEXT_BACK') . '</a>';

			$decrypted_name = '';
			$allDecoded = true;
			foreach (glob($_SERVER['DOCUMENT_ROOT'] . '/' . $arc_name . '*') as $arc_part)
			{
				$decrypted_name = CTar::decryptFile($arc_part, $_REQUEST['EncryptKey'], $_REQUEST['EncryptAlgorithm']);
				if (!$decrypted_name)
				{
					$allDecoded = false;
					break;
			...
			if ($allDecoded)
			{
				$arc_name = substr($arc_name, 0, -4);

				SetCurrentProgress($_REQUEST['encodedFilesCount'], $_REQUEST['encodedFilesCount']);
				$hidden = '<input type="hidden" name="arc_name" value="' . htmlspecialcharsbx($arc_name) . '">';
				showMsg(getMsg('TITLE_PROCESS0'), $status . $hidden, $bottom);
				?><script>reloadPage(2, 1);</script><?
			}
			elseif (!$decrypted_name)
			{
				$hidden = '<input type="hidden" name="EncryptKey" value="' . htmlspecialcharsbx($_REQUEST['EncryptKey']) . '">' .
					'<input type="hidden" name="EncryptAlgorithm" value="' . htmlspecialcharsbx($_REQUEST['EncryptAlgorithm']) . '">' .
					'<input type="hidden" name="encodedFilesCount" value="' . intval($_REQUEST['encodedFilesCount']) . '">' .
					'<input type="hidden" name="arc_name" value="' . htmlspecialcharsbx($arc_name) . '">';
			...
			{
				$encodedFilesCount = count(glob($_SERVER['DOCUMENT_ROOT'] . '/' . $arc_name . '*'));
				SetCurrentProgress($_REQUEST['encodedFilesCount'] - $encodedFilesCount, $_REQUEST['encodedFilesCount']);
				$hidden = '<input type="hidden" name="EncryptKey" value="' . htmlspecialcharsbx($_REQUEST['EncryptKey']) . '">' .
					'<input type="hidden" name="EncryptAlgorithm" value="' . htmlspecialcharsbx($_REQUEST['EncryptAlgorithm']) . '">' .
					'<input type="hidden" name="encodedFilesCount" value="' . intval($_REQUEST['encodedFilesCount']) . '">' .
					'<input type="hidden" name="arc_name" value="' . htmlspecialcharsbx($arc_name) . '">';
...
class CTar
{
	...
	public static function encryptFile($filePath, $key)
	{
		...
		if ($fileSize % 16)
		{
			// 10 + 2 + X + 2 + 4 + 4 = 22 + X
			$padLength = 1;
			while (($fileSize + 22 + $padLength) % 16)
			{
				$padLength++;
			}

			$footer = "\x1f\x8b\x08\x04\x00\x00\x00\x00\x00\x03" // 10 bytes fixed len
				. pack('v', $padLength) // 2 bytes filler length
				. str_repeat('0', $padLength) //filler
				. "\x03\x00" // empty zlib block 2 bytes
				. "\x00\x00\x00\x00" //empty crc 4 bytes
				. "\x00\x00\x00\x00" //zero len 4 bytes
			;

			$f = fopen($filePath, 'ab');
			$written = fwrite($f, $footer);
			fclose($f);

			clearstatcache($filePath);

			if ($written !== strlen($footer))
			{
				return '';
			}
		}

		$md5_key = md5($key); // see CTar::getEncryptKey
		$in = fopen($filePath, 'rb');
		$outName = preg_match('/^(.+)(\.\d+)$/', $filePath, $match) ? $match[1] . '.enc' . $match[2] : $filePath . '.enc';
		$out = fopen($outName, 'wb');
		while ($data = fread($in, 51200))
		{
			$encrypted = static::encrypt($data, $md5_key);
			$written = fwrite($out, $encrypted);
	...
	public static function detectEncryptAlgorithm($filePath, $key)
	{
		$md5_key = md5($key); // see CTar::getEncryptKey
		$in = fopen($filePath, 'rb');
		$header = fread($in, 512);
		foreach (static::getCryptoAlgorithmList() as $encryptAlgorithm)
		{
			$decrypted = static::decrypt($header, $md5_key, $encryptAlgorithm);
			if (
					strpos($decrypted, 'BX') !== false  // gzip
					|| strpos($decrypted, 'bxtar') !== false  // tar
			)
			{
	...
	public static function decryptFile($filePath, $key, $method = null)
	{
		if ($method === null)
		{
			$method = static::detectEncryptAlgorithm($filePath, $key);
		}
		...
		if (!preg_match('/^(.+)(\.enc)(\.\d+)?$/', $filePath, $match))
		{
			return '';
		}
		...
		$md5_key = md5($key); // see CTar::getEncryptKey
		$in = fopen($filePath, 'rb');
		$outName = $match[1] . ($match[3] ?? '');
		$out = fopen($outName, 'wb');
		while ($data = fread($in, 51200))
		{
			$decrypted = static::decrypt($data, $md5_key, $method);
			$written = fwrite($out, $decrypted);
			if ($written !== strlen($data))
			{
				fclose($out);
				fclose($in);
