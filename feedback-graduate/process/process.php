...
if (isset($_POST['lastname'])) {
    $lastname = filter_var($_POST['lastname'], FILTER_SANITIZE_STRING); // защита от XSS
    if (!checkTextLength($lastname, 2, 30)) { // проверка на количество символов в тексте
        $data['lastname'] = 'Поле <b>Фамилия</b> содержит недопустимое количество символов';
        $data['result'] = 'error';
    }
} else {
    $data['lastname'] = 'Поле <b>Имя</b> не заполнено';
    $data['result'] = 'error';
}

if (isset($_POST['middlename'])) {
    $middlename = filter_var($_POST['middlename'], FILTER_SANITIZE_STRING); // защита от XSS
    if (!checkTextLength($middlename, 2, 30)) { // проверка на количество символов в тексте
        $data['middlename'] = 'Поле <b>Отчество</b> содержит недопустимое количество символов';
        $data['result'] = 'error';
    }
} else {
    $data['middlename'] = 'Поле <b>Имя</b> не заполнено';
    $data['result'] = 'error';
}
 


// валидация поля Критерий 1
if (isset($_POST['question1'])) {
    $question1 = filter_var($_POST['question1'], FILTER_SANITIZE_STRING); // защита от XSS
    if (!checkTextLength($middlename, 2, 30)) { // проверка на количество символов в тексте
        $data['question1'] = 'Поле <b>Отношение «обучающийся – руководство»</b> содержит недопустимое количество символов';
        $data['result'] = 'error';
    }
} else {
    $data['question1'] = 'Поле <b>Имя</b> не заполнено';
    $data['result'] = 'error';
}
*/
...
if ($data['result'] == 'success') {
    ...
    $bodyMail = str_replace('%email.lastnameuser%', isset($lastname) ? $lastname : '-', $bodyMail);
    $bodyMail = str_replace('%email.middlenameuser%', isset($middlename) ? $middlename : '-', $bodyMail);
    $bodyMail = str_replace('%email.year_of_issue%', isset($year_of_issue) ? $year_of_issue : '-', $bodyMail);
    $bodyMail = str_replace('%email.speciality%', isset($speciality) ? $speciality : '-', $bodyMail);
    $bodyMail = str_replace('%email.work%', isset($work) ? $work : '-', $bodyMail);
    $bodyMail = str_replace('%email.edu_form%', isset($edu_form) ? $edu_form : '-', $bodyMail);
    //Критерии
    $bodyMail = str_replace('%email.question1%', isset($question1) ? $question1 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question2%', isset($question2) ? $question2 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question3%', isset($question3) ? $question3 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question4%', isset($question4) ? $question4 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question5%', isset($question5) ? $question5 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question6%', isset($question6) ? $question6 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question7%', isset($question7) ? $question7 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question8%', isset($question8) ? $question8 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question9%', isset($question9) ? $question9 : '-', $bodyMail);
    $bodyMail = str_replace('%email.question10%', isset($question10) ? $question10 : '-', $bodyMail);
